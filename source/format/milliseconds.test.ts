	...
	it(title, () => {
		...
		for (const testCase of cases) {
			const [milliseconds, options, expected] =
				testCase.length === 3
					? testCase
					: [testCase[0], undefined, testCase[1]];
...
runTests({
	cases: [
		...
		[
			999,
			{ keepDecimalsOnWholeSeconds: true, secondsDecimalDigits: 0 },
			"0:00",
		],
		[
			999,
			{ keepDecimalsOnWholeSeconds: true, secondsDecimalDigits: 1 },
			"0:00.9",
		],
		[
			999,
			{ keepDecimalsOnWholeSeconds: true, secondsDecimalDigits: 2 },
			"0:00.99",
		],
		[
			999,
			{ keepDecimalsOnWholeSeconds: true, secondsDecimalDigits: 3 },
			"0:00.999",
		],
