...
export const parseCount = (count: string, locale?: string): number => {
	...
	const orders = Array.from({ length: 20 }).map(
		(_, i) =>
			format.formatToParts(10 ** i).find((d) => d.type === "compact")?.value ??
			"",
	);
	const numerals = Array.from({ length: 10 }).map((_, i) => format.format(i));
	const index = new Map(numerals.map((d, i) => [d, i]));
	const minusSign = new RegExp(
		`[${mustExist(parts.find((d) => d.type === "minusSign")).value}]`,
	);
	const decimal = new RegExp(
		`[${mustExist(parts.find((d) => d.type === "decimal")).value}]`,
	);
	const numeral = new RegExp(`[${numerals.join("")}]`, "g");
	const order = new RegExp(
		`(${[...new Set(orders.filter((o) => o !== ""))].join("|")})`,
		"g",
	);
	...
	const value = count
		...
		.replace(minusSign, "-")
		.replace(order, (o) => {
			multiplier = 10 ** orders.indexOf(o);
			return "";
		})
		.replace(/[ \u00a0]/g, "");
