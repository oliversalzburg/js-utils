...
import type {
	CanvasSandboxApplication,
	CanvasSandboxExpectedOptions,
} from "./canvas-sandbox.js";
...
> extends EventTarget {
	...
	constructor(
		...
		this.workerInstance.onmessage = (message: Event) =>
			this.dispatchEvent(
				new CustomEvent(
					(message as MessageEvent<CanvasWorkerMessage<TApplicationOptions>>)
						.data.type,
				),
			);
	}
...
> extends EventTarget {
	...
	constructor(self: typeof globalThis, Kernel: ConstructorOf<TKernel>) {
		...
		self.onmessage = (
			...
			if (message.data.type === "start") {
				const startMessage =
					message.data as CanvasWorkerMessageStart<TApplicationOptions>;
	...
	render = (delta: number): void => {
		this.renderKernel?.onDraw(delta, Date.now());
	};
	...
	reconfigure(
		...
		this.canvas = new Canvas2DHeadless(
